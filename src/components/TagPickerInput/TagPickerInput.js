...
export default function TagPickerInput({
    ...
    */
    ...
    function reducer(state, action) {
        ...
        switch (action.type) {
            case "TOGGLE":
                const newValue = { label: state[action.id].label, selected: true }
                newValue.selected = state[action.id].selected ? false : true;
        ...
        <div>
            ...
            <div className='relative w-72'>
                <div tabIndex="1"
                    ...
                    className=' pt-1.5 pr-6 bg-title shadow border border-gray-900 outline-none rounded flex flex-wrap'
                    style={{ minHeight: 40 }}>
                    {
                        Object.keys(state).filter((id) => {
                            ...
                            .map((id) => {
                                return <Tag key={id} name={state[id].label} action={() => dispatch({ type: "TOGGLE", id: id })} value={name + "_" + state[id].label} />
                            })
                    }
                </div>
                {/* ChevronUp Image */}
            ...
            <div
                ...
                className={`absolute bg-content w-72 border rounded mt-1 border-none shadow-lg border-gray-700 transition-opacity duration-500 transform ${(focusSelect || focusSearch || focusSearchDiv) ? 'opacity-100' : 'opacity-0'} ${isDropdownOpen ? '' : 'hidden'}`}>
                ...
                <div
                    ...
                    className='max-h-32 overflow-auto outline-none'>
                    {
                        <div>
                            {
                                Object.keys(state)
